...
- Each credential blob is encrypted **independently** with its own nonce. There is no document-level "vault seal" step — see §3.4.5 for the rationale.
- Supported algorithms (v1):
  - `xchacha20-poly1305` (recommended default; 192-bit nonce reduces nonce-misuse risk at scale).
  - `aes-256-gcm` (registered for compliance interop; 96-bit nonce).
- Supported key inputs (v1):
  - **Raw 32-byte key** (recommended default). Encoded as base64 on disk and in environment variables. Generated by `alf vault keygen` or any cryptographically secure RNG. Adapters MUST treat the key as opaque bytes; UI tools MAY display only a short fingerprint (e.g., first 4 bytes of SHA-256 of the key) to let users verify they hold the right key without printing it.
  - **Argon2id-derived key** (opt-in for users who prefer a memorable passphrase). The derived 32-byte key is identical in handling to a raw key; only the `kdf` and `kdf_params` fields differ in the on-wire encryption metadata.
- Each credential blob includes an **encryption metadata header** (algorithm identifier, nonce, optional KDF parameters) stored in plaintext alongside the ciphertext.
...
```json
{
  "credential": {
    ...
    "description": "Primary LLM provider key for the kleo agent",
...
The `description` field (optional, plaintext) is a free-form human- and agent-readable string describing what the credential is for (e.g., `"agent-life.run"`, `"primary email for kleo agent"`, `"deployment webhook for production"`). Together with `service`, `credential_type`, `label`, and `tags`, it forms the **plaintext account descriptor** that enables selective decrypt and surgical delete without unsealing the vault (§3.4.5). Privacy implications are discussed in §3.4.6.
...
The `credential_type` field supports: `api_key`, `oauth_token` (with refresh token in the encrypted payload), `webhook_secret`, `session_token`, `ssh_key`, `certificate`, `account`, `custom`.

The `account` type covers provisioned third-party accounts that an agent or user holds — email mailboxes, social accounts, DNS registrations, hosting accounts, and similar. These records typically pair a plaintext descriptor (provider domain, account name) with credentials inside the encrypted payload.
...
#### 3.4.5 Selective Decrypt and Surgical Delete

ALF Layer 4 uses **per-record AEAD encryption**, not the "unseal the vault, work on plaintext, reseal" model common in tools like HashiCorp Vault or 1Password. Each `CredentialRecord` carries its own nonce and `encryption` block; decrypting one record never requires touching another.

This choice is deliberate:

- **Smaller blast radius.** A decryption buffer leak (a logged error, an OOM dump, an LLM context window) exposes only the one credential currently in use, not the whole vault.
- **Least privilege for agents.** Agents driven by an LLM should hold only the secret needed for the immediate action. Per-record encryption makes this the cheap path.
- **Selective restore.** A user importing on a new machine can decrypt and inject only the credentials needed for the agent's current role, leaving the rest as ciphertext on disk.
- **Surgical delete without the key.** Because the plaintext descriptor fields (`id`, `service`, `credential_type`, `description`, `label`, `tags`) are visible to anyone with access to `credentials.json`, an agent or user can drop a single record by ID **without ever holding the vault key**. This is the path used by agents running on ephemeral hosts (e.g., fly.io machines) where the human asks the agent to "delete the email account I provisioned on agent-life.run yesterday": the agent identifies the matching record by descriptor, removes it from the document, and syncs the change. The vault key never enters the conversation.

The cost is per-record metadata (algorithm, nonce, optional KDF params) and the inability to amortize a single KDF derivation across many secrets. For raw-key mode this is free; for passphrase mode, implementations SHOULD cache the derived 32-byte key in memory for the duration of a CLI invocation so users don't pay the Argon2id cost on every record.

#### 3.4.6 Plaintext Descriptor vs Opaque Mode

The plaintext fields on `CredentialRecord` (`service`, `credential_type`, `label`, `description`, `tags`, `capabilities_granted`, timestamps, encryption metadata) are visible to the sync service. This is an **honest disclosure**: the sync service learns that the agent holds, say, an `account` credential of `service: "email"` with `description: "agent-life.run"` and `label: "kleo@agent-life.run"`. It cannot read the password — but it can see that the mailbox exists.

For users who must hide account identifiers (e.g., a privacy-sensitive deployment where even knowing the agent has an email at a specific domain is unacceptable), ALF supports an **opaque mode**:

- Set `credential_type: "custom"`.
- Omit `description`, `label`, and any identifying `tags`.
- Use a generic `service` value such as `"opaque"` or a per-record salt.
- Put every identifying field — username, email address, domain, account ID — inside the encrypted payload under a `kind: "opaque"` envelope (`{ "vault_payload_version": 1, "kind": "opaque", "secret": "..." }` or a richer structure).

In opaque mode, the sync service sees only that a credential exists. Surgical delete still works by `id` (the agent must remember the ID locally, or the user enumerates and decrypts to identify the record they want to remove). The tradeoff is real — opaque mode gives up the ergonomic delete-by-descriptor flow in exchange for stronger metadata privacy. Adapters MUST support both modes; UI tools SHOULD make the tradeoff visible when users create credentials.
